...
def train(resume=0):
    ...
    interpolated = images + alpha*(gen_images-images)
    inte_logit = discriminator(interpolated, isTrain, reuse=True)
    ...
    d_trainer = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0., beta2=0.9).minimize(d_loss, var_list=vars_dis, colocate_gradients_with_ops=True)
    g_trainer = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0., beta2=0.9).minimize(g_loss, var_list=vars_gen, colocate_gradients_with_ops=True)
