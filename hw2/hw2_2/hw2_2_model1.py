                ...
                for l in file:
                    ...
                    if(l == "+++$+++"):
                        ...
                        if((tmp == "+++$+++") or delete):
                            ...
                            delete = ((tmp.count(UNK) > len(tmp)*0.15) or (len(tmp) > 15)) or (len(tmp) < 2)
                        else:
                            #tmp.insert(0,BOS)
...
class model:
    def __init__(self,istest):
        ...
        with tf.variable_scope('encoder') as scope:
            ...
            enc_outputs, encoder_state = tf.nn.dynamic_rnn(
                cell=enc_cell,
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(resume,epoch_num):
                ...
                for step in range(total_steps):
                    ...
                    tmp_loss.append(step_loss)
                    cal_lr = (learning_rate*math.sqrt(1-0.9999**step_num))/(1-0.9**step_num)
                    lr_list.append(cal_lr)
                    ...
                    if(step==total_steps-1):
                        ...
                        for samp in range(3):
                            ...
                            for ind in mydataset.get_ref(samp,False):
                                if(print_text(ind)): break
                            print()
                        loss = np.mean(np.array(tmp_loss))
                        #start validation
                        ...
                        validation_loss = np.mean(np.array(tmp_val_loss))
                ...
                with open('val_loss.txt','a') as val_loss_file:
                    val_loss_file.write(str(validation_loss)+'\n')
                with open('lr_file.txt','a') as file:
                    for r in lr_list:
                        file.write(str(r)+'\n')
                lr_list = []
                ...
                for samp in range(len(test_output)):
                    if(test_output[samp] == None).all():
                        ...
                        for ind in test_output[samp]:
                            ...
                            if(ind == EOS):
                                break
                            elif(ind!=UNK and ind!=PAD and ind!=tmp ):
