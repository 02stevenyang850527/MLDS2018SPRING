...
def discriminator(images, isTrain_tensor, reuse=True):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        score = tf.reshape(tf.nn.sigmoid(conv5), [-1, 1])
        return score
...
def train(resume=0):
    ...
    epsilon = tf.constant(1e-40) # To prevent Nan

    d_loss = -tf.reduce_mean(tf.log(score_real+epsilon) + tf.log(1.-score_fake+epsilon))
    g_loss = tf.reduce_mean(1.-tf.log(score_fake+epsilon))
    ...
    d_trainer = tf.train.AdamOptimizer(learning_rate=2e-4, beta1=0.5).minimize(d_loss, var_list=vars_dis, colocate_gradients_with_ops=True)
    g_trainer = tf.train.AdamOptimizer(learning_rate=2e-4, beta1=0.5).minimize(g_loss, var_list=vars_gen, colocate_gradients_with_ops=True)
